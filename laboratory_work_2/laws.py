...
class Laws:
    ...
    def even(self, data_len: int, max_val) -> np.ndarray:
        """
    ...
    def normal(self, data_len: int, dev, mean: float = 0) -> np.ndarray:
        """
        Генерує дані за нормальним (Гаусівським) законом розподілу.

        :param data_len: Кількість точок даних для генерації.
        :param dev: Стандартне відхилення нормального розподілу.
        :param mean: Середнє значення нормального розподілу.
        :return: Масив з нормально розподіленими даними.
        """

        return np.random.normal(mean, dev, data_len)

    def exponential(self, data_len: int, lam) -> np.ndarray:
        """
